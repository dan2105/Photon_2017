...
void ee_analysis_aahbbar_signal()
{


  TFile *_file0 = new TFile("aabb_background_240GeV_hepmc.root");  //Arquivo formato root
  ...
  vector<fastjet::PseudoJet> exclusive_jets = clust_seq.exclusive_jets(n);
    ...
    //inclusiveJets = clust_seq.inclusive_jets(ptmin);
    //sortedJets    = sorted_by_pt(inclusiveJets);

    vector <fastjet::PseudoJet> sortedjets;
    vector <fastjet::PseudoJet> sortedjetsPT;
    sortedjets = fastjet::sorted_by_E(exclusive_jets);
    sortedjetsPT = fastjet::sorted_by_pt(exclusive_jets);


    //    if(exclusive_jets.size()!=2) continue;


    jet1.SetPxPyPzE(sortedjetsPT[0].px(), sortedjetsPT[0].py(), sortedjetsPT[0].pz(), sortedjetsPT[0].e());
    jet2.SetPxPyPzE(sortedjetsPT[1].px(), sortedjetsPT[1].py(), sortedjetsPT[1].pz(), sortedjetsPT[1].e());
  ...
  for (unsigned int i = 0; i < exclusive_jets.size(); i++) {
    ...
    printf("%5u %15.8f %15.8f %15.8f %8u\n",
	   ...
	   exclusive_jets[i].perp(), (unsigned int) constituents.size());
  ...
  */
  ...
  Double_t  L_int =1; //  ab 
  Double_t sigma_wwinc =1056000;  //pb  pPb   63 TeV
  ...
  TFile* output = new TFile("ee_aabB_analysis_240GeV.root","RECREATE");
  //========================================================================
  h_npart->Write();     
  h_part_id->Write();
  h_part_pt->Write();
